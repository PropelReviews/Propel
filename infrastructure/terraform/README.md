...
- **`SESSION_SECRET`** — generated by Terraform (`random_password` → Secrets
...
| `ZITADEL_CLIENT_SECRET` | When Zitadel OIDC auth is enabled |
| `ZITADEL_MGMT_TOKEN` | When Zitadel org/user provisioning is enabled |
| `TOKEN_ENCRYPTION_KEY` | Fernet key for OAuth tool tokens at rest (Linear, etc.) |

`SESSION_SECRET` is generated by Terraform on first apply (stored in Secrets
Manager at `{name_prefix}/app/SESSION_SECRET`). Do not set it in GitHub or
`terraform.tfvars`.

Non-secret OAuth client IDs and Zitadel issuer/client id (`ZITADEL_ISSUER`,
`ZITADEL_CLIENT_ID`) can live in `app_environment` (GitHub Actions variables).

Set `APP_ENV` to `beta` or `production` in `app_environment` so the API rejects
weak session secrets and missing Zitadel config at startup.
