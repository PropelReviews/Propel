...
async def test_list_tenants(client: AsyncClient):
    await login_test_user(client, "lister@example.com")
...
async def test_soft_delete_admin_only(client: AsyncClient):
    ...
    listed = await client.get("/api/v1/tenants/", headers=act_as_headers(admin_token))
    assert listed.json() == []


@pytest.mark.asyncio
async def test_slug_conflict_returns_409(client: AsyncClient):
    await login_test_user(client, "slug@example.com")
