...
|---|---|---|---|
| POST | `/` | authenticated | Create org; creator gets `owner` |
...
uv sync
# Host:
DATABASE_URL=postgresql://propel:propel@localhost:5432/propel_test SESSION_SECRET=test-secret uv run pytest
# Inside the dev container, swap localhost -> postgres:
DATABASE_URL=postgresql://propel:propel@postgres:5432/propel_test SESSION_SECRET=test-secret uv run pytest
```
