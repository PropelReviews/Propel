...
# Prod account that hosts the shared Zitadel instance (see
# infrastructure/terraform/environments/prod/terraform.tfvars + backend.tf).
EXPECTED_ACCOUNT="616938645090"
...
# Fail fast with an actionable message when the shell is pointed at the wrong AWS
# account/region, instead of a confusing ClusterNotFoundException mid-run.
preflight() {
  local account
  account="$(aws_cli sts get-caller-identity --query Account --output text 2>/dev/null || true)"
  if [[ -z "$account" || "$account" == "None" ]]; then
    cat >&2 <<EOF
ERROR: no AWS credentials resolved for region $REGION.
Authenticate to the prod account ($EXPECTED_ACCOUNT) first, e.g.:
  export AWS_PROFILE=propel-prod        # then: aws sso login --profile propel-prod
EOF
    exit 1
  fi
  if [[ "$account" != "$EXPECTED_ACCOUNT" ]]; then
    cat >&2 <<EOF
ERROR: AWS credentials point at account $account, but prod Zitadel lives in
$EXPECTED_ACCOUNT (region $REGION). Select the prod profile/region and retry:
  export AWS_PROFILE=propel-prod AWS_REGION=us-east-1
EOF
    exit 1
  fi
  local status
  ...
  if [[ "$status" != "ACTIVE" ]]; then
    cat >&2 <<EOF
ERROR: ECS cluster '$CLUSTER' is not ACTIVE in account $account region $REGION
(status: ${status:-none}). Confirm prod infra is applied and you are in the right
account/region before running the reset.
EOF
    exit 1
  fi
  echo "==> Preflight OK (account $account, region $REGION, cluster $CLUSTER)"
}
