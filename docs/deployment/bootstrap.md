...
1. **Environments + Actions variables** — create **`beta`** and **`prod`**
   ...
   | `ZITADEL_MGMT_TOKEN` | When auth enabled | Zitadel IAM_OWNER PAT. prod: the instance admin PAT; beta: the same prod PAT (registers beta's OIDC app on the shared instance) |
