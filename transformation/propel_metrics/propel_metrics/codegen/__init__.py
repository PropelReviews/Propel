...
-- Uniform serving table for declarative metrics (M4+).
-- Compile orchestration swaps per-metric rows atomically (delete+insert).
...
def _render_schema_yml(metrics: list[ResolvedMetric]) -> str:
    lines = [
        ...
        "      Compile job swaps per-metric rows atomically (M4).",
        "    columns:",
        "      - name: tenant_id",
        "        data_tests: [not_null]",
        "      - name: metric_id",
