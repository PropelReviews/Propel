...
grain_day as (
    ...
    with num as (
        select
            ...
            count(*)::float8 as v
        from num_rows r
        group by r.tenant_id, (date_trunc('day', r.t at time zone 'UTC')), r.dim_repo
    ),
    den as (
        select
            r.tenant_id,
    ...
    from den
    left join num using (tenant_id, dim_repo, bucket)
...
grain_week as (
    ...
    with num as (
        select
            ...
            count(*)::float8 as v
        from num_rows r
        group by r.tenant_id, (date_trunc('week', r.t at time zone 'UTC')), r.dim_repo
    ),
    den as (
        select
            r.tenant_id,
...
grain_month as (
    ...
    with num as (
        select
            ...
            count(*)::float8 as v
        from num_rows r
        group by r.tenant_id, (date_trunc('month', r.t at time zone 'UTC')), r.dim_repo
    ),
    den as (
        select
            r.tenant_id,
